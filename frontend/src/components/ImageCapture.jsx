...
export default function ImageCapture({ onIngredientsDetected }) {
  ...
  const detectIngredients = async (imageSrc) => {
    ...
    try {
      ...
      setError(err.response?.data?.error || err.message || 'Terjadi kesalahan saat memproses gambar');
      throw err;
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="w-full space-y-4">
      ...
      <div className="relative w-full aspect-video bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden mb-4 flex items-center justify-center">
        {loading ? (
          <div className="flex flex-col items-center justify-center p-8">
            ...
            <p className="text-gray-600">Mendeteksi bahan...</p>
          </div>
        ) : capturedImage ? (
          <img 
            src={capturedImage} 
          ...
          <div className="text-gray-400 text-center p-8">
            ...
            <p className="text-lg">Belum ada gambar yang diambil atau diunggah</p>
            <p className="text-sm mt-2">Ambil foto atau unggah gambar untuk mendeteksi bahan makanan</p>
          </div>
        )}
        ...
        {loading && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black bg-opacity-70 text-white">
            ...
            <div className="text-xl font-medium">Mendeteksi bahan makanan...</div>
            <p className="text-sm opacity-80 mt-2">Mohon tunggu sebentar</p>
          </div>
        )}
      </div>
      ...
      {showCamera && (
        <div className="fixed inset-0 z-50 bg-black bg-opacity-90 flex flex-col items-center justify-center p-4">
          <div className="relative w-full max-w-4xl aspect-video bg-black rounded-xl overflow-hidden">
            <Webcam
              ...
              onUserMediaError={(err) => {
                ...
                alert("Tidak dapat mengakses kamera. Pastikan Anda telah mengizinkan akses kamera.");
                stopCamera();
              }}
            />
            ...
            <button
              onClick={stopCamera}
              className="absolute top-4 right-4 bg-red-500 hover:bg-red-600 text-white rounded-full w-10 h-10 flex items-center justify-center shadow-lg transition-colors"
              title="Tutup Kamera"
            >
          ...
          <p className="text-white text-center mt-6 max-w-md">
            Posisikan bahan makanan di depan kamera dan tekan tombol bulat untuk mengambil foto
          </p>
        </div>
      )}
      ...
      <p className="text-center text-gray-500 text-sm mt-4">
        Ambil foto bahan makanan atau unggah gambar yang sudah ada
      </p>
